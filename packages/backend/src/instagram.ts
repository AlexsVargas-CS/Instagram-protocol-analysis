...
export class InstagramClient {
    ...
    try {
      ...
      try {
        ...
        process.stderr.write(
          `preLoginSync error (non-fatal): ${preLoginErr instanceof Error ? preLoginErr.message : preLoginErr}\n`,
        );
      }
      ...
      if (error instanceof IgLoginBadPasswordError) {
        ...
        try {
          ...
          process.stderr.write(
            `[challenge] auto() failed: ${challengeErr instanceof Error ? challengeErr.message : challengeErr}\n`,
          );
          throw new AuthenticationError(
        ...
        if (
          ...
          process.stderr.write(
            `[challenge] Fallback checkpoint path (raw response message=${bodyMsg})\n`,
          );
          // Manually set checkpoint state so challenge.auto() can use it.
          (this.ig.state as any).checkpoint = resp;
          const challengeUrl =
            resp?.challenge?.url ?? resp?.checkpoint_url ?? 'https://www.instagram.com/challenge/';
          ...
          try {
            ...
            throw new AuthenticationError(`challenge_url:${challengeUrl}`, 'checkpoint_required');
          }
        }
        throw new InstagramAPIError(error instanceof Error ? error.message : 'Login failed');
      }
    }
  }

  async loadSession(): Promise<User | null> {
    //session is either restored or not.
  ...
  private async runPostLoginFlow(): Promise<void> {
    try {
      ...
      process.stderr.write(
        `postLoginFlow reelsTray error (non-fatal): ${err instanceof Error ? err.message : err}\n`,
      );
    }
    try {
      ...
      process.stderr.write(
        `postLoginFlow timeline error (non-fatal): ${err instanceof Error ? err.message : err}\n`,
      );
    }
  }
  ...
  async getMessages(thread_id: string, _cursor?: string): Promise<GetMessagesResult> {
    try {
      ...
      const messages = items.filter((item) => item != null).map((item) => this.mapMessage(item));
      ...
      if (error instanceof IgLoginRequiredError) {
        ...
        throw new InstagramAPIError(
          error instanceof Error
            ? error.message
            : `Failed to fetch messages for thread ${thread_id}`,
        );
      }
    }
  }
  ...
  async submitTwoFactorCode(code: string): Promise<User> {
    ...
    try {
      ...
      throw new InstagramAPIError(error instanceof Error ? error.message : 'Invalid 2FA code');
    }
  }
  ...
  async startRealtime(
    ...
    for (let attempt = 0; attempt < retries; attempt++) {
      try {
        ...
        debugLog(
          `[realtime] attempt=${attempt + 1} capturedSessionId=${!!capturedSessionId}, cookieUserId=${this.ig.state.cookieUserId}\n`,
        );
  ...
  private mapThread(raw: unknown): Thread {
    ...
    const last_perm_item = raw_last_perm_item
      ? this.mapMessage(raw_last_perm_item)
      : { text: '', timestamp: 0, userId: '' };
