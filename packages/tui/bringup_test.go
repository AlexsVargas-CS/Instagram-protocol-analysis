...
// Synthetic, real-shaped fixtures. These deliberately contain NO real account
// identifiers (PKs, thread IDs, item IDs, or usernames) — the tests only assert
// on structure (thread counts, unread badges, message alignment, sender labels),
// so the exact values are arbitrary placeholders.
const (
	mePK      = "100000000001"
	themPK    = "100000000002" // thread1 contact
	them2PK   = "100000000003" // thread2 contact
	thread1   = "1000000000000000000000000000000000000001"
	thread2   = "1000000000000000000000000000000000000002"
	meUser    = "test_me"
	themUser  = "test_contact_a"
	them2User = "test_contact_b"
)
...
func realThreads() []Thread {
	return []Thread{
		{
			ThreadID: thread1,
			Users:    []User{{PK: themPK, Username: themUser}},
			LastMessage: Message{ItemId: "item_thread1_last",
				Text: "test message four", Timestamp: 1780439453120656, UserId: themPK},
			UnreadCount: 0, LastActivityAt: 1780439453120000,
		},
		{
			ThreadID:    thread2,
			Users:       []User{{PK: them2PK, Username: them2User}},
			LastMessage: Message{Text: "test preview", Timestamp: 1780430000000000, UserId: them2PK},
...
func realMessages() []Message {
	return []Message{
		{ItemId: "a1", Text: "test message one", Timestamp: 1780430621592009, UserId: mePK},
		{ItemId: "a2", Text: "test message two", Timestamp: 1780430628275993, UserId: themPK},
		{ItemId: "a3", Text: "test message three", Timestamp: 1780430646314125, UserId: mePK},
		{ItemId: "a4", Text: "test message four", Timestamp: 1780439453120656, UserId: themPK},
	}
}
...
func TestM2_ThreadsAndConversationRender(t *testing.T) {
	...
	frame := m.View()
	t.Log("\n=== BROWSE (thread list) ===\n" + frame)
	for _, want := range []string{themUser, them2User, "CHATS", "Connected as " + meUser} {
		if !strings.Contains(frame, want) {
			t.Errorf("thread list missing %q", want)
		}
	}
	// thread2 has 2 unread — the pink badge should show "2".
	if !strings.Contains(frame, "2") {
		t.Errorf("expected unread badge for %s", them2User)
	}
	...
	for _, want := range []string{
		"test message one", // my message
		"test message two", // their message
		"test message four",
		"✓✓",            // read-receipt marker on my (right-aligned) bubbles
		"● " + themUser, // green sender label on their bubble
	} {
	...
	myIdx := strings.Index(conv, "test message one")
	theirIdx := strings.Index(conv, "test message two")
...
func TestM2_RealtimeMessageAppends(t *testing.T) {
	...
	incoming := Message{ItemId: "z1", Text: "live ping", Timestamp: 1780440000000000, UserId: them2PK}
...
func TestM4_ExpiredSessionPromptsRelogin(t *testing.T) {
	...
	for _, tc := range cases {
		...
		if m.usernameInput.Value() != meUser {
